...
- (void)downloadWithURL:(NSURL *)url delegate:(id<SDWebImageManagerDelegate>)delegate options:(SDWebImageOptions)options
{
    ...
    //NSLog(@"downloadWithURL cache count: %d", cacheURLs.count); 
}
...
- (void)imageCache:(SDImageCache *)imageCache didNotFindImageForKey:(NSString *)key userInfo:(NSDictionary *)info
{
    ...
    // stats
    if (downloaders.count > _downloadersMax) {
        _downloadersMax = downloaders.count;
    }
    //NSLog(@"downloaders: %d, max: %d", downloaders.count, _downloadersMax);
}
...
- (void)imageDownloader:(SDWebImageDownloader *)downloader didFinishWithImage:(UIImage *)image
{
    ...
    for (NSInteger idx = (NSInteger)[downloaders count] - 1; idx >= 0; idx--)
    {
        ...
        if (aDownloader == downloader)
        {
            ...
            {
                // added 10.7.11 pb
                if ([delegate respondsToSelector:@selector(webImageManager:didFinishWithImageDownloaded:)])
                {
                    [delegate performSelector:@selector(webImageManager:didFinishWithImageDownloaded:) withObject:self withObject:image];
                }
            }
            else
            {
