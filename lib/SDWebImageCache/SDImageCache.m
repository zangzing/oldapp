...
/*
 ...
 changed the memCache model
 now has a high-water mark (maxMemCacheBytes) and trims the caches (oldest first) when max is exceeded
 
 added memCacheItem, memCacheBytes
 added imageBytes, trimMemCache
...
static unsigned long long maxMemCacheBytes = 2*1024*1024;
static BOOL wantMemCache = YES;
...
- (id)init
{
    if ((self = [super init]))
    {
        NSLog(@"SDImageCache init");

        // Init the memory cache
        memCache = [[NSMutableDictionary alloc] init];
        memCacheItems = [[NSMutableArray alloc] init];
        memCacheBytes = 0;
...
- (void)dealloc
{
    ...
    [memCacheItems release], memCacheItems = nil;
...
+(unsigned long long) imageBytes:(UIImage*)image
{
    // approximate image size in bytes (worst case)
    // added pb 2.12.11
    
    return  (unsigned long long)(image.size.width * image.size.height / 8);
}


-(void)trimMemCache
{
    // remove oldest mem cache items until under maxMemCacheBytes bytes
    // added pb 12.2.11
    
    //NSLog(@"image memCache #: %d; bytes: %llu", memCache.count, memCacheBytes);
    
    int trimCount = 0;
    unsigned long long trimBytes = 0;
    
    while (memCacheBytes > maxMemCacheBytes && memCacheItems.count > 0) {
        NSString *key = [memCacheItems objectAtIndex:0];
        UIImage *image = [memCache objectForKey:key];
        
        unsigned long long bytes = [SDImageCache imageBytes:image];
        memCacheBytes -= bytes;
        
        // remove head from cache
        [memCacheItems removeObjectAtIndex:0];
    ...
    /*
    if (trimCount>0)
        NSLog(@"trimming memCache: images/bytes: %d/%llu; now at %d/%llu", trimCount, trimBytes, memCache.count, memCacheBytes);
    */
}
...
- (void)notifyDelegate:(NSDictionary *)arguments
{
    ...
    if (image)
    {
        if (wantMemCache) {
            ...
            [memCacheItems addObject:key];
            memCacheBytes += [SDImageCache imageBytes:image];
            [self trimMemCache];
        }
...
- (void)removeImageForKey:(NSString *)key
{
    ...
    memCacheBytes -= [SDImageCache imageBytes:image];
    [memCache removeObjectForKey:key];
    [memCacheItems removeObject:key];
